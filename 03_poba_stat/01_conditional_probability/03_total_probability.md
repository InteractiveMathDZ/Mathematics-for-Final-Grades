...
elements:
  ...
  - parags:
      - rtl: "قبل بناء الدستور الكلي، يجب ضبط الأرضية النظرية: نقول عن مجموعة من الحوادث $B_1, B_2, \\dots, B_n$ أنها تشكل <b>تجزئة تامة</b> للفضاء العيني $\\Omega$ إذا حققت شرطين ميكانيكيين:"
      - rtl: "• <b>التنافي المثنى:</b> أي حادثتين مختلفتين هما متنافيتان تماماً ($B_i \\cap B_j = \\emptyset$ عند $i \\neq j$)."
      - rtl: "• <b>الشمولية المطلقة:</b> اتحاد كل هذه الحوادث يعيد تشكيل الفضاء الكامل $\\Omega$ بلا أي فراغ ($B_1 \\cup B_2 \\cup \\dots \\cup B_n = \\Omega$)."

  - h3: "2. دستور الاحتمالات الكلية (Law of Total Probability)"
  - parags:
      - rtl: "إذا كانت لدينا حادثة عشوائية $A$ مستعرضة تتقاطع مع حوادث التجزئة التامة، فإن احتمال وقوعها الإجمالي يُحسب ميكانيكياً بتجميع احتمالات تقاطعاتها الصافية مع كل جزء من التجزئة."
      - rtl: "بلغة شجرة الاحتمالات، الاحتمال الكلي للحادثة $A$ هو <b>مجموع احتمالات جميع المسارات المنتهية بالحادثة $A$</b>:"
      - center: "$P(A) = \\sum_{i=1}^{n} P(A \\cap B_i) = P(B_1) \\times P_{B_1}(A) + P(B_2) \\times P_{B_2}(A) + \\dots + P(B_n) \\times P_{B_n}(A)$"
