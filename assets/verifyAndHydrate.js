...
function updateExerciseRecord(exID, valuesStr, evaluation, version) {
  ...
  console.log(
    `تم حفظ التمرين ${exID} بنجاح. المحاولة رقم: ${evaluation.attempts}`,
  );
}
...
function hydrateLessonProgress() {
  ...
  if (progressBar && progressVal) {
    ...
    if (resume[lessonID]) {
      ...
      console.log(
        `تم إنعاش تقدم المحور [${lessonID}] بنسبة: ${finalPercentage}%`,
      );
    }
  }
}
...
function evaluateAnswers(exID, allElements) {
  ...
  const newCumulativeAverage =
    (oldAverage * oldCount + attemptScore) / newCount;
...
function applyVisuals(exID, evaluation, allElements) {
  ...
  if (progressVal)
    progressVal.innerText = Math.round(evaluation.avgScore) + "/10";
