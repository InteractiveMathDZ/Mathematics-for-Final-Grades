...
function buildDirectedAngle(id, config) {
  ...
  const O = board.create("point", [0, 0], {
    ...
    size: 3,
    fixed: true,
    label: { color: theme.textColor, offset: [-15, -15] },
  });
...
function buildTrigonometricCircle(id, config) {
  ...
  const center = board.create("point", centerPos, {
    visible: false,
    fixed: true,
  });
    ...
    board.create(
      ...
      [
        ...
        function () {
          ...
          return (
            "θ = " + angleDeg.toFixed(0) + "°  │  " + piFraction + " π rad"
          );
        },
      ],
  ...
  if (config.show_projections) {
    const cosPoint = board.create(
      "point",
      [
        function () {
          return movingPoint.X();
        },
        0,
      ],
      { visible: false },
    );
    board.create("segment", [movingPoint, cosPoint], {
      strokeColor: theme.projectionLine,
      strokeWidth: 1,
      dash: 2,
    });
    board.create("segment", [center, cosPoint], {
      strokeColor: "#00ffcc",
      strokeWidth: 3.5,
      name: "",
    });

    const sinPoint = board.create(
      "point",
      [
        0,
        function () {
          return movingPoint.Y();
        },
      ],
      { visible: false },
    );
    board.create("segment", [movingPoint, sinPoint], {
      strokeColor: theme.projectionLine,
      strokeWidth: 1,
      dash: 2,
    });
    board.create("segment", [center, sinPoint], {
      strokeColor: "#ff0077",
      strokeWidth: 3.5,
      name: "",
    });
